package com.davidlong.traiker;

public class StringConsts {
    public static final String GJRW = "高级软卧";
    public static final String RW = "软卧一等卧";
    public static final String YDW = "软卧一等卧";
    public static final String RZ = "软座";
    public static final String TDZ = "特等座";
    public static final String WZ = "无座";
    public static final String YW = "硬卧二等卧";
    public static final String EDW = "硬卧二等卧";
    public static final String YZ = "硬座";
    public static final String EDZ = "二等座";
    public static final String YDZ = "一等座";
    public static final String SWZ = "商务座";
    public static final String DW = "动卧";

    public static final String BLANK="";
    public static final String WU="无";
    public static final String YOU="有";

    public static final String Y="Y";
    public static final String N="N";

    public static final String SPLITER="\\|";
    public static final String TIME_SPLITER=":";

}
